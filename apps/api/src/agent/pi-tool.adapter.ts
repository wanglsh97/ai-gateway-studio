 ...
 *
 * - JSON Schema 参数作为 Pi 工具 schema（Pi 负责按 schema 校验模型参数）。
 * - 成功结果映射为 Pi tool result（text content + details 携带 summary/audit）。
 * - 失败按 Pi 约定抛错；details 通过错误对象透传以便上层持久化审计。
 */
export function toPiAgentTool(definition: AgentToolDefinition): AgentTool {
  return {
    ...
    execute: async (
      ...
      const result = await definition.execute(params as Record<string, unknown>, {
        toolCallId,

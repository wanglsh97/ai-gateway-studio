...
单模型 Agent 会话使用 `@assistant-ui/react` 的 LocalRuntime 以及 Thread、Message、Composer 和 ActionBar primitives 管理客户端消息状态、自动滚动、取消和无障碍交互。LocalRuntime 的自定义 `ChatModelAdapter` 只把 assistant-ui 的文本消息映射到仓库内 `@aigateway/sdk`，继续消费既有 POST SSE 契约；Web 不引入 Vercel AI SDK、assistant-ui Cloud 或厂商协议。多模型对比仍由 `@aigateway/sdk` compare helper 管理独立请求和 AbortController，避免 UI runtime 改变已确认的每路隔离语义。

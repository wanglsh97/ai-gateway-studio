...
### Decision 12: Configured model catalog is separate from provider adapters

Chat 客户端提交公开模型实例 ID，而不是厂商 alias。服务端 `ModelCatalog` 通过 `CHAT_MODELS` 配置把实例 ID 映射为社区展示名、`qwen/glm/deepseek/kimi` 厂商 alias 和真实上游模型 ID；Adapter registry 仍按厂商注册，因此同一 Kimi Adapter 可以运行 Kimi K2.6、Kimi K3 等多个实例。未配置 `CHAT_MODELS` 时从现有 `*_MODEL_ID` 环境变量生成每厂商一个实例，保证已有部署兼容。模型发现接口只公开模型目录元数据，不公开 API Key、Base URL 或其他鉴权配置。
...
3. **用户端网页**：包含公开首页/登录页、GitHub 登录保护的 Chat 单模型/对比、文生图、Prompt 优化、响应式布局和主题。

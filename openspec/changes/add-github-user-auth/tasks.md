...
- [ ] 1.1 更新 Prisma schema，新增 `User`、`UserSession`，为 `RequestLog` 和 `ImageGenerationTask` 增加必填用户关系、唯一约束和筛选索引
- [ ] 1.2 创建正式 Prisma migration，并提供迁移前清理匿名开发测试记录的显式步骤，不在迁移中静默猜测或回填用户
- [ ] 1.3 扩展 API 配置与 `.env.example`，校验 GitHub client、callback、Session secret 和 30 天固定有效期，确保 secret 不进入日志或响应
